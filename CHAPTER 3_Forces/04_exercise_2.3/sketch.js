...
function draw() {
  ...
  /*
  // by chatGPT
  let distanceThreshold = 50;  // increased threshold
  let distanceA = moverA.position.y + moverA.diameter / 2 - height;
  let distanceB = moverB.position.y + moverB.diameter / 2 - height;

  if (distanceA > -distanceThreshold) {
    let strengthA = map(distanceA, 0, -distanceThreshold, 0, 2);  // much stronger force
    moverA.applyForce(createVector(0, -strengthA));
  
    // optional damping to prevent sticking
    if (moverA.position.y + moverA.diameter / 2 > height) {
      moverA.velocity.y *= 0.7;
    }
  }

  if (distanceB > -distanceThreshold) {
    let strengthB = map(distanceB, 0, -distanceThreshold, 0, 2);
    moverB.applyForce(createVector(0, -strengthB));
  
    if (moverB.position.y + moverB.diameter / 2 > height) {
      moverB.velocity.y *= 0.7;
    }
  }
  */
