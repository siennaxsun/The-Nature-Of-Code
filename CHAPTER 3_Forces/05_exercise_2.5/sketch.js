/*Create a wind force that’s variable. Can you make it interactive? For example, think of a fan located where the mouse is and pointed toward the circles.*/

let moverA;
let moverB;


function setup() {
  createCanvas(800, 400);
  
  let massA = 1;
  let massB = 2;
  let startingPosA = width/2 - 300;
  let startingPosB = width/2 + 300;
  let diameterA = 50;
  let diameterB = 100;
  
  moverA = new Mover(startingPosA, massA, diameterA);
  moverB = new Mover(startingPosB, massB, diameterB);
}

function draw() {
  background(220);
  
  // apply gravity to moverA and moverB
  let gravity = createVector(0, 0.1);
  moverA.applyForce(gravity);
  moverB.applyForce(gravity);
  
  // apply windforce to moverA and moverB
  let mouse = createVector(mouseX, mouseY)
  let windThreshold = 0.1;
  
  let distanceToA = dist(mouseX, mouseY, moverA.position.x, moverA.position.y);
  // the larger the distance, the smaller wind strength
  let windAStrength = map(distanceToA, 0, width, windThreshold, 0); 
  // wind blow from mouse to moverB
  let dirToA = p5.Vector.sub(moverA.position, mouse);
  dirToA.normalize();
  dirToA.mult(windAStrength);
  
  let distanceToB = dist(mouseX, mouseY, moverB.position.x, moverB.position.y);
  // wind strength to B based on its distance to the mouse
  let windBStrength = map(distanceToB, 0, width, windThreshold, 0);
  // wind from mouse to moverB
  let dirToB = p5.Vector.sub(moverB.position, mouse)
  dirToB.normalize();
  dirToB.mult(windBStrength);
  
  // activate wind simulation when mouse is clicked
  if (mouseIsPressed){
    moverA.applyForce(dirToA);
    moverB.applyForce(dirToB);
    // visualize wind direction
    stroke(0);
    line(mouseX, mouseY, moverA.position.x, moverA.position.y);
    line(mouseX, mouseY, moverB.position.x, moverB.position.y);
  }
  
  

  // apply a push back force from the edge of the wall
  let distanceThreshold = 30;
  let distanceA = abs(moverA.position.y + moverA.diameter/2 - height);
  let distanceB = abs(moverB.position.y + moverB.diameter/2 - height);
  
  if (distanceA < distanceThreshold){
    let pushbackAX = map(distanceA, 0, distanceThreshold, 0, 2 * moverA.mass);
    let pushbackA = createVector(0, pushbackAX*(-1));
    moverA.applyForce(pushbackA);
  }
  
  if (distanceB < distanceThreshold){
    let pushbackBX = map(distanceB, 0, distanceThreshold, 0, moverB.mass);
    let pushbackB = createVector(0, pushbackBX*(-1));
    moverB.applyForce(pushbackB);
  }
  
  
  
  // update moverA and moverB
  moverA.update();
  moverA.show();
  moverB.update();
  moverB.show();

  
}


class Mover{
  constructor(startingPosX, mass, diameter){
    this.diameter = diameter;
    this.position = createVector(startingPosX, 20 + this.diameter/2);
    this.velocity = createVector();
    this.acceleration = createVector();
    this.topspeed = 5;
    this.mass = mass
  }
  
  applyForce(force){
    let f = force.copy();
    f.div(this.mass);
    this.acceleration.add(f);
    
  }
  
  update(){
    
    this.velocity.add(this.acceleration);
    // this.velocity.limit(this.topspeed);
    this.position.add(this.velocity);
    
    this.bounce();
    
    //reset accleration
    this.acceleration.mult(0);
  }
  
  bounce(){
    if (this.position.y + this.diameter/2 >= height){
      this.velocity.mult(-1);
    }
    
    if (this.position.x + this.diameter/2 >= width){
      this.position.x = width - this.diameter/2;
      this.velocity.x *= -1;
    } else if (this.position.x + this.diameter/2 <= 0){
      this.position.x = this.diameter/2;
      this.velocity.x *= -1;
    }
    
  }
  
  show(){
    fill(200);
    stroke(0);
    strokeWeight(2);
    circle(this.position.x, this.position.y, this.diameter);
  }
}



// first version

// let moverA;
// let moverB;


// function setup() {
//   createCanvas(800, 400);
  
//   let massA = 1;
//   let massB = 2;
//   let startingPosA = width/2 - 300;
//   let startingPosB = width/2 + 300;
//   let diameterA = 50;
//   let diameterB = 100;
  
//   moverA = new Mover(startingPosA, massA, diameterA);
//   moverB = new Mover(startingPosB, massB, diameterB);
// }

// function draw() {
//   background(220);
  
//   // apply gravity to moverA and moverB
//   let gravity = createVector(0, 0.1);
//   moverA.applyForce(gravity);
//   moverB.applyForce(gravity);
  
//   // apply windforce to moverA and moverB
//   let windThreshold = 100;
//   let windToA = dist(mouseX, moverA.position.x, mouseY, moverA.position.y);
//   let windAX = map(noise(windToA), 0, windThreshold, 0.01, 0.05);
//   let windA = createVector(windAX, 0);
//   if (mouseX > moverA.position.x){
//     windA.mult(-1);
//   }
  
//   let windToB = dist(mouseX, moverB.position.x, mouseY, moverB.position.y);
//   let windBX = map(noise(windToB), 0, windThreshold, 0.01, 0.05);
//   let windB = createVector(windBX, 0);
//   if (mouseX > moverB.position.x){
//     windB.mult(-1);
//   }
  
//   moverA.applyForce(windA);
//   moverB.applyForce(windB);
  

//   // apply a push back force from the edge of the wall
//   let distanceThreshold = 30;
//   let distanceA = abs(moverA.position.y + moverA.diameter/2 - height);
//   let distanceB = abs(moverB.position.y + moverB.diameter/2 - height);
  
//   if (distanceA < distanceThreshold){
//     let pushbackAX = map(distanceA, 0, distanceThreshold, 0, 2 * moverA.mass);
//     let pushbackA = createVector(0, pushbackAX*(-1));
//     moverA.applyForce(pushbackA);
//     // moverA.velocity.y *= 0.7;
//   }
  
//   if (distanceB < distanceThreshold){
//     let pushbackBX = map(distanceB, 0, distanceThreshold, 0, moverB.mass);
//     let pushbackB = createVector(0, pushbackBX*(-1));
//     moverB.applyForce(pushbackB);
//     // moverB.velocity.y *= 0.7;
//   }
  
  
  
//   // update moverA and moverB
//   moverA.update();
//   moverA.show();
//   moverB.update();
//   moverB.show();
// }

// class Mover{
//   constructor(startingPosX, mass, diameter){
//     this.diameter = diameter;
//     this.position = createVector(startingPosX, 20 + this.diameter/2);
//     this.velocity = createVector();
//     this.acceleration = createVector();
//     this.topspeed = 5;
//     this.mass = mass
//   }
  
//   applyForce(force){
//     let f = force.copy();
//     f.div(this.mass);
//     this.acceleration.add(f);
    
//   }
  
//   update(){
    
//     this.velocity.add(this.acceleration);
//     // this.velocity.limit(this.topspeed);
//     this.position.add(this.velocity);
    
//     // this.bounce();
    
//     //reset accleration
//     this.acceleration.mult(0);
//   }
  
//   bounce(){
//     if (this.position.y + this.diameter/2 >= height){
//       this.velocity.mult(-1);
//     }
    
//     if (this.position.x + this.diameter/2 >= width){
//       this.position.x = width - this.diameter/2;
//       this.velocity.x *= -1;
//     } else if (this.position.x + this.diameter/2 <= 0){
//       this.position.x = this.diameter/2;
//       this.velocity.x *= -1;
//     }
    
//   }
  
//   show(){
//     fill(200);
//     stroke(0);
//     strokeWeight(2);
//     circle(this.position.x, this.position.y, this.diameter);
//   }
// }