...
class Flock{
  ...
  addBoids(){
    for (let i = 0; i<this.total; i++){
      this.boids[i] = new Boid(random(width), random(height));
    }
  }
...
class Boid{
  ...
  separate(boids){
    ...
    let distThreshold = this.size * 5; // how close is too close
  ...
  align(boids){
    let minThreshold = this.size*5;
    let maxThreshold = this.size * 10;
    let summedDir = createVector();
    let count = 0;
    for (let other of boids){
      let dist = p5.Vector.dist(other.position, this.position);
      if (this !== other && dist <= maxThreshold){
  ...
  cohere(boids){
    let distThreshold = this.size * 10;
    let summedDir = createVector();
    let count = 0;
    for (let other of boids){
      let dist = p5.Vector.dist(this.position, other.position);
